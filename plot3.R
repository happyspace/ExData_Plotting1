...
plot3 <- function(){
    ...
    plot(pcdates$dt, pcdates$Sub_metering_1, type = 'l', ylab = "Energy sub metering", xlab = "")
    with(pcdates, lines(dt, Sub_metering_2, col=2))
    with(pcdates, lines(dt, Sub_metering_3, col=3))
    legend("topright", lty=1, col = c("black", "red", "green"), legend = c("Sub metering 1", "Sub metering 2", "Sub metering 3"))
    dev.off()
}